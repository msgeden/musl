...
unsigned long __register_mac(const unsigned char *m, unsigned len)
{
    ...
    res.byte[2] = (uint8_t)(state[0] >> 8);
    res.byte[3] = (uint8_t)(state[0] >> 0);
    res.byte[4] = (uint8_t)(state[1] >> 24);
    res.byte[5]= (uint8_t)(state[1] >> 16);
    res.byte[6] = (uint8_t)(state[1] >> 8);
    res.byte[7] = (uint8_t)(state[1] >> 0);

    //64-bit truncate of SHA256
    //long hash = (b1) | (b2 << 8) | (b3 << 16) | (b4 << 24) | (b5 << 32) | (b6 << 40) | (b7 << 48) | (b8 << 56);
    return res.longlong;
}
